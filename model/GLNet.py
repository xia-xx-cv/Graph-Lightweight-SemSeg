...
from model.GraphHead import GraphHead
...
class ChannelWiseConv(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride=1):  ############################## ksize？？看看是不是1
        """
        Args:
            nIn: number of input channels
...
class ContextGuidedBlock(nn.Module):  # GRL
    def __init__(self, nIn, nOut, dilation_rate=2, reduction=16, add=True):
        ...
        self.F_loc = ChannelWiseConv(n, n, 3, 1)  # local feature，kernel_size是3
...
class GCAttn(nn.Module):
    """
    This class corresponds part 3 in the paper.
    """
            ...
            if classname.find('Conv2d') != -1:
                ...
                if m.bias is not None:
                    ...
                    if m.bias is not None:
                        m.bias.data.zero_()
        ab = m.weight  # 256
        self.head = GraphHead(256, 128, classes)
        self.head3 = nn.Conv2d(128, 256, kernel_size=1)
        self.down = nn.AvgPool2d(3, stride=2, padding=1)
        self.qkv = CAM_Module(128)
    ...
    def forward(self, input):
        ...
        for i, layer in enumerate(self.level2):
            if i == 0:
                ...
                if i == 1:
                    extract_feature.append(self.down(output1))
        ...
        for i, layer in enumerate(self.level3):
            if i == 0:
                ...
                if i == 5 or i == 10 or i == 16:
                    extract_feature.append(output2)

        # context corresponds to eq.(1) in the paper
        context = self.qkv(output2_0, extract_feature[1], extract_feature[2], extract_feature[3])


        output2_cat = self.bn_prelu_3(torch.cat([output2_0, output2], 1) + self.head3(context))

        # Add a graph convolution module to the end of GCAttn, which corresponds to part 3.1 in the paper
        out3_graph = self.head(output2_cat)

        # upsample segmenation map ---> the input image size
        out = F.interpolate(out3_graph, input.size()[2:], mode='bilinear',
                         align_corners=False)
        return  out


if __name__ == '__main__':
    a = torch.randn(1, 3, 680, 680)
    model = GCAttn()
    b = model(a)
    print(b.size())
