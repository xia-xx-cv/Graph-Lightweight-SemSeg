...
#user
from model import GLNet  # network
...
def train_model(args):
    ...
    M = args.M
    N = args.N
    model = GLNet.GCAttn(classes= args.classes, M= M, N= N)
    network_type="GLNet"
    print("=====> current architeture: GLNet")
    ...
    args.savedir = ( args.savedir + args.dataset + '/'+ network_type +"_M"+ str(M) + 'N' +str(N) + 'bs' 
                    + str(args.batch_size)+ 'gpu' + str(0)+ "_"+str(args.train_type)+'/')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default = "GLNet", help = "model name: A Graph-involved Lightweight Semantic Segmentation Network")
    parser.add_argument('--dataset', default = "cityscapes", help = "dataset: cityscapes or camvid")
    parser.add_argument('--data_dir', default = "xx", help ='data directory')
    parser.add_argument('--dataset_list', default = "cityscapes_train_list.txt",
                        help = "train and val data, for computing the ration of all kinds, mean and std")
    parser.add_argument('--train_data_list', default = "./dataset/list/Cityscapes/cityscapes_train_list.txt", help = "train set")
    parser.add_argument('--train_type', type = str, default = "ontrain",
                         help = "ontrain for training on train set, ontrain for training on train+val set")
    parser.add_argument('--max_epochs', type = int, default = 452, help = "the number of epochs: 300 for train set, 350 for train+val set")
    ...
    parser.add_argument('--resume', type = str, default = "./checkpoint/cityscapes/modelbest.pth",
